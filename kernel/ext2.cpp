...
void Ext2::ReadInode(Inode inode, uint32_t offset, size_t count, char *ptr) {
	...
	size_t block_ptr_end = block_ptr_begin + (((offset + count) % block_size) != 0) + 1;

	char buffer[1024]; // XXX: assuming block size is 1024 again
	for(size_t b = block_ptr_begin; b < block_ptr_end; ++b) {
		uint32_t block_ptr = this->GetBlockAddr(inode.block_ptr[b]);
		ata_pio_read(this->lba + block_ptr / 512, block_size / 512, &buffer);
		for(size_t n = 0; n < count && offset % block_size + n < block_size; ++n) {
			ptr[(b - block_ptr_begin) * block_size + n] = buffer[offset % block_size + n];
		}
	}
}
